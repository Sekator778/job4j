...
Лямбда-выражения сами по себе не выполняются. Они  формируют реализацию абстрактного метода, определяемого функциональным интер­
фейсом, который задает свой целевой тип
...
Функциональные интерфейсы имеют тесную связь с лямбда выражениями и служат как основа для применения лямбда выражений 
в функциональном программировании на Java. Хотелось бы напомнить один нюанс — 
до появления Java 8 все методы в интерфейсе неявно считались абстрактными.
 С выходом JDK 8 появилось такое понятие как метод по умолчанию. Метод по умолчанию – это метод объявленный в интерфейсы, 
 поведение которого предопределено, иначе говоря, метод уже имеет реализацию в интерфейсе.
...
оно должно быть обьявленно в абстрактном методе функциональном интерфейсе, либо можно поймать исключение в теле лямбда-выражения.
...
это — поддержка функционального стиля операций над потоками, такими как обработка и «свёртка» обработанных данных.
...
Допустим мы хотим найти сумму цену тех телефонов, у которых цена меньше определенного значения.
...
Технически, var — это не ключевое слово, а зарезервированное имя типа

[к оглавлению](#FP-Labmda-Stream-API)
    
## В какие случаях можно использовать var    
var - может использоваться только в сочетании с данными + цикл for.
для «локальных объявлений переменных с инициализаторами»
